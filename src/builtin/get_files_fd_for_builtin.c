...
/*   Updated: 2024/05/28 12:45:28 by stouitou         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "minishell.h"

static bool	open_infile(t_exe *exe, char *file)
{
	if (exe->io_fd[0] != -1)
		close (exe->io_fd[0]);
...
static bool	open_outfile(t_exe *exe, char *file)
{
	if (exe->io_fd[1] != -1)
		close (exe->io_fd[1]);
...
static bool	open_app_outfile(t_exe *exe, char *file)
{
	if (exe->io_fd[1] != -1)
		close (exe->io_fd[1]);
	if (access(file, F_OK) == 0)
...
int	get_files_fd_for_builtin(t_exe *exe, t_files *file)
{
	...
	while (file)
	{
		...
		{
			if (!open_infile(exe, file->content))
				return (0);
		}
		if (file->category == OUTFILE)
		{
			if (!open_outfile(exe, file->content))
				return (0);
		}
		if (file->category == APP_OUTFILE)
		{
			if (!open_app_outfile(exe, file->content))
				return (0);
		}
		file = file->next;
	}
